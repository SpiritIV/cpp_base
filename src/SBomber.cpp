...
{
	// game->DeleteObj(game_object);
    auto    it = vec_objects->begin();

    for (; it != vec_objects->end(); it++)
    {
        if (*it == game_object)
        {
            vec_objects->erase(it);
            break;
        }
    }
}
...
void    DropBomb::Execute()
{
    ...
    if (*bombsNumber > 0)
    {
        ...
        game_object->SetDirection(0.3, 1);
        game_object->SetSpeed(2);
        game_object->SetPos(x, y);
        game_object->SetWidth(SMALL_CRATER_SIZE);

        vec_objects->push_back(game_object);
        --(*bombsNumber);
        (*score) -= Bomb::BombCost;
    }
}
...
void SBomber::CheckBombsAndGround() 
{
	...
	for (size_t i = 0; i < vecBombs.size(); i++)
	{
		if (vecBombs[i]->GetY() >= y) // ����������� ����� � ������
		{
			...
			Command	*del_it = new  DeleteDynamicCommand(vecBombs[i], &vecDynamicObj);
			CommandExecuter(del_it);
...
void SBomber::CheckDestoyableObjects(Bomb * pBomb)
{
	...
	for (size_t i = 0; i < vecDestoyableObjects.size(); i++)
	{
        ...
        if (vecDestoyableObjects[i]->isInside(x1, x2))
        {
			...
			Command	*del_it = new DeleteCommand(vecDestoyableObjects[i], &vecStaticObj);
    ...
    case 'b':
		{
			...
			Command *drop_bomb = new DropBomb(pPlane, pBomb, &bombsNumber, &vecDynamicObj, &score);
			CommandExecuter(drop_bomb);
		}
		break;
    case 'B':
		{
